...
const Page = () => {
  ...
  const busCapacity = parseInt(busType.split(" ")[0]) || 48;
  const tripDepartureTime = "08:00 AM";
  const tripArrivalTime = "12:00 PM";
  // const busExtras = "Wi-Fi, AC";
  const busExtras = ["Air Conditioning","Refrigirator", "Power Outlets","Television"];
  ...
  return (
    <>
      <main className='flex-1  border-t border-b bg-white dark:bg-slate-700  flex flex-col items-center w-full relative overflow-hidden '>
        {/* <h1 className='text-black text-2xl mb-'>Bus Seat Selection</h1> */}
        <div className='flex flex-row w-full max-sm:gap-5  max-sm:flex-col-reverse sm:max-md:flex-col-reverse'>
          <BusDetails
            busImage={busImage}
            busNumber={busNumber}
            busCompany={busType}
          ...
          />
          <section className='bg-white flex flex-col items-center  p-5 w-full rounded-lg overflow-y-hidden '>
            {isBooked === false ? (
              <>
                <h2 className='text-lg mb-3'>Seats available</h2>
                <SeatSelection
                  busCapacity={busCapacity}
                  selectedSeats={selectedSeats}
